...
class DeliveryManager extends Singleton {
	...
	public function __construct( array $settings ) {
        ...
        add_action( 'manage_posts_custom_column', [ $this, 'custom_posts_column' ], 10, 2 );
	}
    ...
    function add_posts_columns( $columns ) {
        ...
        foreach ( $this->get_services() as $service ) {
            foreach ( $service->get_target_post_types() as $pt ) {
                if ( !in_array( $pt, $target_post_types ) ) {
                    $target_post_types[] = $pt;
                }
            }
        }
...
//            echo ( $cf_example ) ? $cf_example : '－';
            ...
            $checked_list = get_post_meta( $post_id, $this->meta_name, true );
...
<ul>
EOM;
                foreach ( $checked_list as $service ) {
                    echo <<<EOM
    <li>
        ・{$service}
    </li>
...
EOM;
	...
	public function callback_checkbox_template() {
		...
		// 全チェック/全非チェックのボタン.
		echo <<<EOS
	<div id="trs-feed-button">
		<button class="check">All Check</button>
		<button class="uncheck">All Uncheck</button>
	</div>
EOS;
	...
	<div>
		<label>
			<input type="checkbox" name="{$meta_name}[]" value="{$id}" {$is_checked}>{$label}
		</label>
	</div>
EOS;
			}
